...
static string tempnam(const char* ignore_dir, const char* prefix) {
  ...
  CHECK_NE(0, result);
  string windows_path = quantsystem::FromWindowsStr(path);
  return quantsystem::FromWindowsPath(windows_path);
}
#endif
...
void CreateTestingTempDir() {
  default_tempdir_.reset(new string(tempnam(NULL, "quantsystem")));
