...
TEST_F(DateTimeTestFixture, TestConstructor) {
  ...
  DateTime date_parse = DateTime::ParseDate("20120103");
  EXPECT_TRUE(date_parse.is_valid());
  EXPECT_EQ("2012-01-02T16:00:00Z", date_parse.ToString());

  DateTime date_parse_long = DateTime::ParseDate("2012-01-03", true);
  EXPECT_TRUE(date_parse_long.is_valid());
  EXPECT_EQ("2012-01-02T16:00:00Z", date_parse_long.ToString());

  DateTime date_ymd(2012, 1, 3);
  EXPECT_TRUE(date_ymd.is_valid());
  EXPECT_EQ("2012-01-02T16:00:00Z", date_ymd.ToString());
}
