...
#include "quantsystem/common/base/stringprintf.h"
...
DateTime DateTime::ParseDate(const string& date, bool long_format) {
  struct tm utc;
...
DateTime DateTime::DateTimeInvalid() {
  DateTime d = DateTime();
  d.MarkInvalid();
  return d;
}
...
DateTime::DateTime(const Year& year, const Month& month, const Day& day) {
  struct tm utc;
  memset(&utc, 0, sizeof(utc));
...
TimeSpan DateTime::TimeOfDay() const {
  struct timeval time = t_;
  struct tm utc;
  gmtime_r(&time.tv_sec, &utc);
  utc.tm_year = 0;
  utc.tm_mon = 0;
  utc.tm_mday = 0;
  time.tv_sec = timegm(&utc);
...
DateTime& DateTime::operator +=(const TimeSpan& span) {
  ...
  span.GetTimeval(&timeval);
  t_.tv_usec += timeval.tv_usec;
  t_.tv_sec += timeval.tv_sec;
}

DateTime& DateTime::operator -=(const TimeSpan& span)  {
  struct timeval time;
  span.GetTimeval(&time);
  t_.tv_sec -= time.tv_sec;
  t_.tv_usec -= time.tv_usec;
}
...
TimeSpan DateTime::operator -(const DateTime& date) const {
  struct timeval time;
  time.tv_sec = t_.tv_sec - date.t_.tv_sec;
  time.tv_usec = t_.tv_usec - date.t_.tv_usec;
  return TimeSpan(time);
}
}  // namespace quantsystem
