...
#ifndef QUANTSYSTEM_COMMON_TIME_DATE_TIME_H_
#define QUANTSYSTEM_COMMON_TIME_DATE_TIME_H_
...
class DateTime {
 public:
  ...
  /**
   * Parse a date from string format YYYYMMDD.
   * @return An invalid date if the specified time is not valid.
   */
  static DateTime ParseDate(const string& date, bool long_format = false);

  /**
   * Construct an invalid date.
   * @return An invliad date.
   */
  static DateTime DateTimeInvalid();
  ...
  /**
   * Contruct a date from year, month, and day.
   */
  DateTime(const Year& year, const Month& month, const Day& day);
  ...
  /**
   * Convert the data to an yyyymmdd format string.
   */
  ...
  /**
   * Increment date by the given TimeSpan.
   */
  DateTime& operator +=(const TimeSpan& span);

  /**
   * Decrement date by the given TiemSpan.
   */
  DateTime& operator -=(const TimeSpan& span);

  /**
   * @return a new date incremented by the given TimeSpan.
   */
  DateTime operator +(const TimeSpan& span) const;

  /**
   * @return a new date decremented by the given TimeSpan.
   */
  DateTime operator -(const TimeSpan& span) const;

  /**
   * @return the TimeSpan between two date.
   */
  TimeSpan operator -(const DateTime& date) const;
...
#endif  // QUANTSYSTEM_COMMON_TIME_DATE_TIME_H_
