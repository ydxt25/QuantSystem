...
#include "quantsystem/common/strings/util.h"
...
/* static */
common::util::Status File::RecursivelyCreateDirWithPermissions(
  ...
  while (!to_do.empty()) {
    ...
    if (mkdir(new_path.c_str(), permissions) != 0) {
      common::util::Status status(common::util::error::UNKNOWN,
...
common::util::Status File::Flush() {
#ifdef _MSC_VER
  return util::Status();
#else
  do {
    if (fsync(fd_) == 0) return common::util::Status();  // OK
  } while (errno == EINTR || errno == EAGAIN);
  return common::util::Status(
      common::util::error::UNKNOWN,
...
common::util::Status File::WriteString(const StringPiece& bytes) {
  ...
  for (ssize_t len = 0, remaining = bytes.size();
    ...
    if (len < 0) {
      ...
      return common::util::Status(common::util::error::DATA_LOSS, "Error writing to file");
    }
  }
  return common::util::Status();  // OK
}

common::util::Status File::Read(char* buffer, int64 len, int64* got) {
  for (*got = 0; *got < len; /* empty */) {
    ...
    if (in < 0) {
      ...
      common::util::Status status =
          common::util::Status(common::util::error::UNKNOWN,
...
/* static */
common::util::Status File::WritePath(const string& path, const StringPiece& data) {
  File* file = File::Open(path, "wb");
  if (!file) {
    return common::util::Status(common::util::error::INVALID_ARGUMENT,
                        "Could not write to file");
  }
  common::util::Status status = file->WriteString(data);
  file->Close();
  return status;
}

common::util::Status File::ReadPath(const string& path, string* s) {
  string native_path = ToNativePath(path);
  s->clear();
  ...
  if (fd < 0) {
    ...
    return common::util::Status(common::util::error::NOT_FOUND, error);
  }
  ...
  do {
    ...
    if (len == -1) {
      ...
      string error = strerror(errno);
      LOG(ERROR) << error;
      status = common::util::Status(common::util::error::DATA_LOSS, error);
      break;
    } else if (!len) {
      break;
    }
