...
#include "quantsystem/common/base/template_util.h"
#include "quantsystem/common/base/type_traits.h"
#include "quantsystem/common/util/algorithm.h"
namespace quantsystem {
// Calls delete on the item in the queue
template<typename T>
void STLDeleteMapElements(T* container) {
  if (!container) return;
...
// Sorts and removes duplicates from a vector or deque.
template<typename T>
...
// this file. It exists only to implement the following functions:
...
inline void STLStringResizeUninitialized(string* s, size_t new_size) {
  if (sizeof(*s) == sizeof(InternalStringRepGCC4)) {
    ...
    // The line below depends on the layout of 'string'. THIS IS NON-PORTABLE
    // CODE. If our STL implementation changes, we will need to change this as
    // well.
...
template <typename T> struct IsFunc : common::base::false_type {};
template <typename R>
struct IsFunc<R()> : common::base::true_type {};
template <typename R, typename T1>
struct IsFunc<R(T1)> : common::base::true_type {};
template <typename R, typename T1, typename T2>
struct IsFunc<R(T1, T2)> : common::base::true_type {};
template <typename R, typename T1, typename T2, typename T3>
struct IsFunc<R(T1, T2, T3)> : common::base::true_type {};
template <typename R, typename T1, typename T2, typename T3, typename T4>
struct IsFunc<R(T1, T2, T3, T4)> : common::base::true_type {};
...
void STLSetDifference(const In1& a, const In2& b, Out* out, Compare compare) {
  ...
  assert(common::util::gtl::is_sorted(a.begin(), a.end(), compare));
  assert(common::util::gtl::is_sorted(b.begin(), b.end(), compare));
...
typename common::base::enable_if<!common::util::gtl::stl_util_internal::IsFunc<Out>::value,
                         void>::type
STLSetDifference(const In1& a, const In2& b, Out* out) {
  STLSetDifference(a, b, out, common::util::gtl::stl_util_internal::TransparentLess());
}
...
Out STLSetDifferenceAs(const In1& a, const In2& b) {
  return STLSetDifferenceAs<Out>(
      a, b, common::util::gtl::stl_util_internal::TransparentLess());
}
...
STLSetUnion(const In1& a, const In2& b, Out *out) {
  return STLSetUnion(a, b, out,
                     common::util::gtl::stl_util_internal::TransparentLess());
}
template<typename Out, typename In1, typename In2, typename Compare>
...
In1 STLSetUnion(const In1& a, const In2& b) {
  return STLSetUnion(a, b, common::util::gtl::stl_util_internal::TransparentLess());
}
template<typename In1>
...
STLSetSymmetricDifference(const In1& a, const In2& b, Out* out) {
  return STLSetSymmetricDifference(
      a, b, out, common::util::gtl::stl_util_internal::TransparentLess());
}
template<typename Out, typename In1, typename In2, typename Compare>
...
bool SortedRangesHaveIntersection(InputIterator1 begin1, InputIterator1 end1,
                                  ...
                                  Comp comparator) {
  assert(common::util::gtl::is_sorted(begin1, end1, comparator));
  assert(common::util::gtl::is_sorted(begin2, end2, comparator));
