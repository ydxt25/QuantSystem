...
class File {
 public:
  ...
  static common::util::Status RecursivelyCreateDirWithPermissions(
      const string& path, mode_t permissions);
  static common::util::Status ReadPath(const string& path, string* s);
  static common::util::Status WritePath(const string& path, const StringPiece& s);
  ...
  common::util::Status Flush();
  common::util::Status Write(const char* buffer, int64 length) {
    return WriteString(StringPiece(buffer, length));
  }
  common::util::Status WriteString(const StringPiece& bytes);
  common::util::Status ReadToString(string* output);
  common::util::Status Read(char* buffer, int64 length, int64* got);
